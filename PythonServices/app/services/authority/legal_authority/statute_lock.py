...
def _infer_statute_from_paragraph(question: str) -> Optional[Dict[str, Any]]:
    ...
    # Check context for statute hints
...
def lock_statute(question: str) -> Dict[str, Any]:
    ...
    # Score each statute based on patterns and keywords
    ...
    if best_domain[1] >= 0.4:  # Threshold for doctrinal inference
        ...
        if config:
            return {
                ...
                'statute': statute,
                'domain': config['domain'],
                'source': 'doctrinal_fallback',
                'confidence': best_domain[1],
                'inference_reason': f'Doctrinal inference based on domain scoring'
            }
    
    # Cannot determine statute
    from .clarifications import missing_statute_clarification
    return {
        'status': 'MISSING',
