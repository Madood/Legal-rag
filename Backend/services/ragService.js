...
class RAGService {
  constructor() {
    ...
    console.log('✅ RAGService initialized with STATUTE-FIRST ARCHITECTURE with AUTHORITY METADATA');
  }
  ...
  async generateResponse(question, allDocuments, options = {}) {
    ...
    if (authority.status !== 'LOCKED') {
      ...
      return this.handleAuthorityFailure(authority, question, startTime, language);
    }
    
    const { statute, field, source, questionType } = authority;
    console.log(`🔒 [Authority] Locked: ${statute} (${field}), source: ${source}`);
    
    // ⭐⭐ STEP 2: QUESTION CLASSIFICATION
    const classification = questionClassifier.classify(question, statute);
    console.log(`🎯 [Classification] ${classification.type}, ${classification.language}, ${classification.complexity}`);
    
    // ⭐⭐ STEP 3: DOCTRINE & SYSTEM HANDLING (bypass retrieval)
    if (classification.type === 'DOCTRINE') {
      return this.handleDoctrineQuestion(question, statute, classification, startTime, language);
    }
    
    if (classification.type === 'SYSTEM') {
      return this.handleSystemQuestion(question, classification, startTime, language);
    }
    
    // ⭐⭐ STEP 4: FIELD ISOLATION
    const statuteDocuments = legalFieldDetector.isolateField(field, allDocuments);
    
    if (statuteDocuments.length === 0) {
      console.log(`⚠️  No documents for statute: ${statute}`);
      return this.handleMissingSourceQuestion(question, statute, field, startTime, language);
    }
    
    // ⭐⭐ STEP 5: LEGAL REFERENCE EXTRACTION
    const legalReference = legalAuthorityService.extractLegalReference(question);
    
    console.log(
      `📜 [Reference] ${legalReference.requestedReference ? (legalReference.isArticle ? 'Article' : '§') : ''}${legalReference.requestedReference || 'None'}`
    );
    ...
    ragDocuments.forEach(doc => {
      if (doc.chunks && Array.isArray(doc.chunks)) {
        doc.chunks.forEach((chunk, index) => {
          ...
          const sourceAuthority = sourceAuthorityResolver.classifyChunk(content, statute, doc);
          ...
          if (statute === 'HGB' && isBoilerplate) {
            ...
            if (this.containsNormativeContent(content)) {
              console.log(`   ⚠️ HGB chunk marked as boilerplate but contains normative content`);
            }
          }
          ...
          allChunks.push({
            ...
            documentName: doc.filename || doc.metadata?.title || `Document ${index}`,
    ...
    if (statute === 'HGB') {
      ...
      allChunks = allChunks.filter(chunk => {
        ...
        const hasLegalRef = /§\s*\d+/.test(content) || /(?:Artikel|Art\.|Article)\s*\d+/.test(content);
        const hasNormative = this.containsNormativeContent(content);
        
        return hasLegalRef || hasNormative;
      });
      console.log(`🧹 HGB special handling: Removed ${originalCount - allChunks.length} boilerplate chunks`);
    } else {
      allChunks = allChunks.filter(chunk => !chunk.isBoilerplate);
      console.log(`🧹 Removed ${originalCount - allChunks.length} boilerplate chunks`);
    }
    ...
    if (legalReference.requestedReference) {
      const matchingChunks = allChunks.filter(chunk => {
        ...
        if (legalReference.isArticle) {
          const articlePattern = new RegExp(`(?:Artikel|Art\\.|Article)\\s*${legalReference.requestedReference}\\b`, 'i');
          return articlePattern.test(content);
        } else {
          const paragraphPattern = new RegExp(`§\\s*${legalReference.requestedReference}\\b`, 'i');
      ...
      if (matchingChunks.length > 0) {
        ...
        console.log(`✅ Found ${allChunks.length} chunks with exact ${legalReference.isArticle ? 'Article' : '§'}${legalReference.requestedReference} match`);
      } else {
        console.log(`⚠️  No exact match for ${legalReference.isArticle ? 'Article' : '§'}${legalReference.requestedReference}`);
        
        // Try partial matching for HGB
        if (statute === 'HGB') {
          const partialPattern = new RegExp(`${legalReference.requestedReference}`, 'i');
          const partialMatches = allChunks.filter(chunk => 
            partialPattern.test(chunk.content || '')
          );
          
          if (partialMatches.length > 0) {
            console.log(`⚠️  Found ${partialMatches.length} partial matches for HGB §${legalReference.requestedReference}`);
    ...
    console.log(`🔍 Found ${similarChunks.length} similar chunks in ${statute}`);
    ...
    if (!statuteValidation.valid) {
      console.log(`⚠️  Statute requirements failed: ${statuteValidation.reason}`);
      ...
      if (statute === 'HGB') {
        ...
        if (similarChunks.length > 0) {
          console.log(`✅ HGB lenient validation passed - found ${similarChunks.length} chunks`);
          statuteValidation.valid = true;
          statuteValidation.reason = 'Lenient validation applied for HGB';
        }
      } else {
        return this.handleStatuteValidationFailure(question, statuteValidation, startTime, language);
      }
    }
    ...
    if (exactReferenceMatch && legalReference.requestedReference) {
      ...
      validation = legalAuthorityService.validateAnswer(question, similarChunks[0]?.content || '', statute);
    }
    ...
    if (!validation.isValid) {
      ...
      if ((statute === 'HGB' || statute === 'BGB') && 
          (question.includes('§') || question.match(/(?:Artikel|Art\.)\s*\d+/))) {
        console.log(`⚠️  Overriding validation for ${statute} with specific reference`);
        validation.isValid = true;
        validation.message = 'Validation overridden for statute with specific reference';
      }
    }
    
    if (!validation.isValid) {
      return this.handleJudicialValidationFailure(question, validation, startTime, language, similarChunks);
    }
    ...
    const topChunks = this.selectDiverseTopChunks(rankedChunks, this.MAX_CHUNKS);
    ...
    topChunks.forEach((chunk, i) => {
      const authorityInfo = chunk.metadata?.authorityType ? 
        `[${chunk.metadata.authorityType} Rank:${chunk.metadata.authorityRank || 0}]` : '';
      console.log(`   ${i+1}. Similarity: ${chunk.similarity?.toFixed(3) || 'N/A'} ${authorityInfo}, Preview: ${chunk.content?.substring(0, 80)}...`);
    });
    
    // ⭐⭐ STEP 14: EXTRACT LEGAL RULE
    const legalRule = this.extractLegalRuleFromChunks(topChunks, statute, question);
    ...
    const citations = this.prepareEnhancedCitationsWithAuthority(topChunks.slice(0, 3), statute);
    
    // ⭐⭐ STEP 17: CALCULATE CONFIDENCE WITH AUTHORITY WEIGHT
    const confidence = this.calculateEnhancedConfidenceWithAuthority(
      {
        avgScore: topChunks.reduce((sum, chunk) => sum + (chunk.similarity || 0), 0) / topChunks.length || 0,
        chunkCount: topChunks.length,
        avgAuthorityRank: topChunks.reduce((sum, chunk) => sum + (chunk.metadata?.authorityRank || 0), 0) / topChunks.length || 0
      },
      exactReferenceMatch,
      legalReference.requestedReference,
    ...
    const safetyValidation = await safetyCheck.validateBeforeAnswer(question, response);
    response.safetyCheck = safetyValidation;
    
    console.log(`✅ Answer generated for ${statute} with confidence: ${confidence.toFixed(2)}`);
    console.log(`✅ Statute locked: ${response.metadata.statuteLocked}`);
    console.log(`✅ Architecture: ${response.metadata.architecture}`);
    console.log(`✅ Judicial validation: ${validation.isValid ? 'PASS' : 'FAIL'} - ${validation.message}`);
    console.log(`✅ Average authority rank: ${response.metadata.authority_stats?.average_rank?.toFixed(2) || 'N/A'}`);
  ...
  rerankWithAuthorityAndStatutePriority(chunks, question, requestedReference, statute, isArticle) {
    return chunks.sort((a, b) => {
      const scoreA = this.calculateEnhancedChunkScore(a, question, requestedReference, statute, isArticle);
      const scoreB = this.calculateEnhancedChunkScore(b, question, requestedReference, statute, isArticle);
      return scoreB - scoreA;
    });
  }

  calculateEnhancedChunkScore(chunk, question, requestedReference, statute, isArticle) {
    ...
    if (requestedReference) {
      let pattern;
      if (isArticle) {
        pattern = new RegExp(`(?:Artikel|Art\\.|Article)\\s*${requestedReference}\\b`, 'i');
      } else {
  ...
  prepareEnhancedCitationsWithAuthority(chunks, statute) {
    return chunks.map((chunk, index) => {
      ...
      if (content.includes('§')) {
        ...
        const artMatch = content.match(/(?:Artikel|Art\.|Article)\s*\d+[a-z]?\b/);
        if (artMatch) reference = artMatch[0];
      }
      
      let excerpt = content;
      const boilerplatePatterns = [
        'Übersetzung', 'Translation', 'register notices', 'Commercial Register',
        'Ein Service des Bundesministerium', 'Service provided by',
  ...
  calculateEnhancedConfidenceWithAuthority(statuteData, exactReferenceMatch, requestedReference, 
                                         requirementsSatisfied, legalRule, statute, chunks) {
    ...
    if (/§\s*\d+/.test(legalRule) || /(?:Artikel|Art\.|Article)\s*\d+/.test(legalRule)) {
      confidence += 0.15;
    }
    ...
    const highAuthorityChunks = chunks.filter(chunk => 
  ...
  buildResponseWithAuthorityMetadata(answer, citations, confidence, documentsUsed, 
                                    chunksUsed, startTime, statute, field, 
                                    classification, legalReference, exactReferenceMatch,
                                    statuteValidation, judicialValidation, chunks) {
    ...
    return {
      ...
      metadata: {
        ...
        referenceMatchNotes: exactReferenceMatch ? 
          `Found exact match for ${legalReference.isArticle ? 'Article' : '§'}${legalReference.requestedReference}` :
  ...
  generateStructuredAnswerWithAuthority(question, legalRule, chunks, language, statute, reference) {
    if (language !== 'german') {
      return this.generateEnglishAnswerWithAuthority(chunks, statute, reference, legalRule, question);
    }
  ...
  generateAuthorityContext(chunks, statute) {
    ...
    const avgRank = authorityRanks.length > 0 ? 
      (authorityRanks.reduce((a, b) => a + b, 0) / authorityRanks.length).toFixed(1) : 'N/A';
  ...
  generateEnglishAnswerWithAuthority(chunks, statute, reference, legalRule, question) {
    const statuteName = this.statuteDisplayNames[statute] || statute;
  ...
  extractStatuteFromDoc(doc) {
    ...
    if (filename.includes('stgb') || content.includes('Strafgesetzbuch')) return 'StGB';
    if (filename.includes('bgb') || content.includes('Bürgerliches Gesetzbuch')) return 'BGB';
    if (filename.includes('hgb') || content.includes('Handelsgesetzbuch')) return 'HGB';
    if (filename.includes('gg') || content.includes('Grundgesetz') || content.includes('Basic Law')) return 'GG';
    if (filename.includes('gdpr') || filename.includes('dsgvo') || content.includes('Datenschutz-Grundverordnung') || content.includes('REGULATION (EU) 2016/679')) return 'EU-GDPR';
  ...
  isBoilerplateContent(text) {
    ...
    const boilerplatePatterns = [
      'Übersetzung', 'Translation', 'register notices', 'Commercial Register',
      'Ein Service des Bundesministerium', 'Service provided by',
      'Copyright', 'CELEX', 'Official Journal', 'Amtsblatt',
      'gesetze-im-internet.de', 'juris', 'reproduced', 'PDF generated',
      'Samson Übersetzungen', 'Dr. Carmen', 'Michael Bohlander',
  ...
  containsLegalTerms(text) {
    ...
    const legalTerms = [
      'Verpflichtung', 'Haftung', 'Schaden', 'Schadensersatz', 'Ansprüche',
      'Rechte', 'Pflichten', 'Vertrag', 'Kauf', 'Miete', 'Dienstleistung',
      'Kaufmann', 'Handelsgeschäft', 'Gewerbe', 'Grundrecht', 'Straftat',
      'Strafe', 'Freiheitsstrafe', 'Geldstrafe', 'Daten', 'Personenbezogen',
      'Verarbeitung', 'Einwilligung'
    ];
  ...
  extractDoctrineName(question) {
    ...
    if (lowerQuestion.includes('verhältnismäßigkeitsprinzip')) return 'verhältnismäßigkeitsprinzip';
    if (lowerQuestion.includes('rechtsstaatsprinzip')) return 'rechtsstaatsprinzip';
    
    return 'general';
  }

  validateStatuteRequirements(chunks, statute, question) {
    const requirements = this.statuteRequirements[statute] || { requiresParagraph: false };
    ...
    if (requirements.requiresParagraph && !hasParagraph) {
      return {
        ...
        chunkPreview: chunks.length > 0 ? chunks[0].content?.substring(0, 100) : 'No chunks'
      };
    }
    ...
    console.log(`✅ Statute requirements satisfied: Found ${paraCount} paragraphs, ${artCount} articles`);
  ...
  extractLegalRuleFromChunks(chunks, statute, question) {
    ...
    for (const chunk of chunks) {
      ...
      const sectionMatch = content.match(/(§\s*\d+[a-z]?\s+[^.!?]{20,150}[.!?])/);
      if (sectionMatch) {
      ...
      for (const sentence of sentences) {
        ...
        for (const pattern of normativePatterns) {
          if (pattern.test(trimmed)) {
            console.log(`   ✅ Found normative statement: ${trimmed.substring(0, 80)}...`);
            return trimmed + '.';
          }
        }
      }
      ...
      for (const chunk of chunks) {
        ...
        for (const sentence of sentences) {
          ...
          if (trimmed.length > 20 && trimmed.includes('§')) {
            console.log(`   ⚠️ HGB fallback with §: ${trimmed.substring(0, 80)}...`);
            return trimmed + '.';
          }
        }
      }
  ...
  generateMeaning(ruleText, statute, chunks, question) {
    ...
    if (statute === 'GG') {
      if (allContent.includes('Artikel 5') || allContent.includes('Article 5')) {
        return 'Grundrecht der Meinungsfreiheit gemäß Artikel 5 Grundgesetz.';
      }
      if (allContent.includes('Artikel 38') || allContent.includes('Article 38')) {
    ...
    if (statute === 'HGB') {
      ...
      if (question.includes('Handelsbrief') || question.includes('kaufmännisches Bestätigungsschreiben')) {
    ...
    if (statute === 'EU-GDPR') return 'Regelt den Umgang mit personenbezogenen Daten innerhalb der EU.';
  ...
  generateEffect(ruleText, statute, chunks, question) {
    ...
    if (statute === 'StGB') {
      if (ruleLower.includes('freiheitsstrafe') || ruleLower.includes('gefängnis')) {
    ...
    if (statute === 'BGB') {
      if (ruleLower.includes('schaden') || ruleLower.includes('schadensersatz') || 
    ...
    if (statute === 'GG') return 'Verfassungsrechtlicher Schutz mit Vorrang vor einfachem Recht.';
    if (statute === 'EU-GDPR') return 'Datenschutzrechtliche Compliance-Verpflichtungen mit Bußgeldern.';
  ...
  handleDoctrineQuestion(question, statute, classification, startTime, language) {
    console.log('⚖️  Processing as LEGAL DOCTRINE question');
    
    const doctrineName = this.extractDoctrineName(question);
    const doctrine = legalAuthorityService.getDoctrineExplanation(doctrineName, language);
  ...
  handleMissingSourceQuestion(question, missingStatute, field, startTime, language) {
    console.log(`⚠️  Missing ${missingStatute} for ${field} question`);
    
    const clarification = clarificationService.generateMissingSourceClarification(
      missingStatute, 
      language
    );
  ...
  handleJudicialValidationFailure(question, validation, startTime, language, chunks = []) {
    ...
    if (chunks.length > 0) {
      ...
      if (content && content.length > 50) {
        ...
        if (language === 'german') {
          return {
            answer: `**Validierungswarnung**\n\n` +
                    `Die Validierung hat Bedenken gemeldet: ${validation.message || 'Ungültige Antwort'}\n\n` +
            ...
            citations: this.prepareEnhancedCitationsWithAuthority(chunks.slice(0, 2), validation.expectedStatute || 'unknown'),
            confidence: 0.3,
            documentsUsed: 1,
            metadata: {
  ...
  getNoDocumentsResponse(startTime, language, statute) {
    ...
    if (language === 'german') {
      return {
        answer: `**Keine ${statuteName}-Dokumente verfügbar**\n\n` +
    ...
    return {
      answer: `**No ${statuteName} documents available**\n\n` +
  ...
  getNoContentResponse(startTime, language, statute) {
    ...
    if (language === 'german') {
      return {
        answer: `**Keine Textabschnitte für ${statuteName} gefunden**\n\n` +
    ...
    return {
      answer: `**No text chunks found for ${statuteName}**\n\n` +
  ...
  getNoRelevantContentResponse(startTime, language, statute) {
    ...
    if (language === 'german') {
      return {
        answer: `**Keine relevanten Textstellen in ${statuteName} gefunden**\n\n` +
    ...
    return {
      answer: `**No relevant text found in ${statuteName}**\n\n` +
